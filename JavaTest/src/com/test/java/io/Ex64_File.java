...
public class Ex64_File {
	...
	public static void main(String[] args) {
		...
		/*
		 * 1. 파일/디렉토리 조작 - 윈도우 탐색기로 하는 모든 행동 > 자바로 구현 - 파일 > 정보 확인, 새로 만들기, 이름 바꾸기,
		 * 이동하기, 삭제하기 등 - 폴더 > 정보 확인, 새로 만들기, 이름 바꾸기, 이동하기, 삭제하기 등
		 * 
		 * 2. 파일 입출력 - 파일 > 읽기/쓰기 - 메모장 기능
		 */
...
//			}
	...
	private static void m7() {
		...
		if(!dir.exists()) {
			...
			System.out.println(dir.mkdirs()); //연관된 폴더 모두 
		}
	}
	...
	private static void m1() {
		...
		if (file.exists()) {
			...
			System.out.println(file.length()); // 0 > 파일 크기 바이트
			System.out.println(file.getAbsolutePath()); // 경로
			System.out.println(file.lastModified()); // 수정한 날짜 1691379515616
			System.out.println(file.isHidden()); // false 숨긴항목여부
